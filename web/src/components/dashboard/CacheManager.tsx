  ...
  return (
    <div className="glass-card p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h3 className="text-lg font-semibold text-white">Cache & Rate Limits</h3>
      ...
      {stats && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          <div className="bg-midnight/50 rounded-lg p-3">
            <div className="text-2xl font-bold text-white">{stats.cached_platforms}</div>
            <div className="text-xs text-silver">Cached</div>
          </div>
          <div className="bg-midnight/50 rounded-lg p-3">
            <div className="text-2xl font-bold text-warning-orange">{stats.expired_platforms}</div>
            <div className="text-xs text-silver">Expired</div>
          </div>
          <div className="bg-midnight/50 rounded-lg p-3">
            <div className="text-2xl font-bold text-danger-red">{stats.rate_limited_platforms}</div>
            <div className="text-xs text-silver">Rate Limited</div>
          </div>
          <div className="bg-midnight/50 rounded-lg p-3">
            <div className="text-2xl font-bold text-cyber-blue">{stats.total_platforms}</div>
      ...
      {cacheData && (
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            {Object.entries(cacheData.cache_status).map(([platform, status]) => {
              ...
              return (
                ...
                >
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-silver">
                          {status.has_cache ? (
                            <>
                              Cached {status.cached_at && new Date(status.cached_at).toLocaleTimeString()}
                              {expired && ' (Expired)'}
                            </>
                          ) : (
                            'No cache'
                  ...
                  {platform === 'twitter' && cacheData.rate_limits[platform] && (
                    <div className="mt-3 pt-3 border-t border-midnight">
                      <div className="text-xs text-silver mb-2">Rate Limits:</div>
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                        {Object.entries(cacheData.rate_limits[platform]).map(([endpoint, limit]: [string, any]) => (
                          <div key={endpoint} className="bg-midnight/30 rounded p-2">
                            <div className="text-xs font-medium text-white">{endpoint}</div>
                            <div className="text-xs text-silver">
                              {limit.request_count || 0} requests
                            </div>
                            {limit.limit_reached && (
                              <div className="text-xs text-danger-red">
                                Reset: {formatTimeUntilReset(platform, endpoint)}
                              </div>
                            )}
                          </div>
                        ))}
      ...
      <div className="mt-6 p-4 bg-cyber-blue/10 rounded-lg border border-cyber-blue/20">
        <div className="flex items-start space-x-2">
          ...
          <div className="text-sm text-silver">
            <p className="mb-2">
              <strong className="text-white">Cache:</strong> Stores API responses to reduce requests and improve performance.
            </p>
            <p>
              <strong className="text-white">Rate Limits:</strong> API providers limit requests per time window. 
              When limits are reached, cached data is used instead.
            </p>
          </div>
        </div>
      </div>
