...
export function StatsGrid({ socialAccounts, selectedPlatform = 'overview', onRefresh }: StatsGridProps) {
  ...
  const fetchMetrics = async () => {
    setLoading(true);
    try {
      const platformParam = selectedPlatform === 'overview' ? '' : `?platform=${selectedPlatform}`;
  ...
  const totalFollowers = summary?.total_followers || metrics.reduce((sum, m) => sum + m.followers_count, 0);
  const totalPosts = summary?.total_posts || metrics.reduce((sum, m) => sum + m.posts_count, 0);
  const avgEngagementRate = summary?.avg_engagement_rate || (metrics.length > 0
  ...
  const stats: Array<{
    ...
    {
      name: selectedPlatform === 'overview' ? "Connected Platforms" : `${getPlatformDisplayName()} Status`,
      value: selectedPlatform === 'overview' ? connectedPlatforms : (metrics.length > 0 ? "Connected" : "Not Connected"),
      change: selectedPlatform === 'overview'
        ? (connectedPlatforms > 0 ? `+${connectedPlatforms}` : "0")
        : (metrics.length > 0 ? "Active" : "Inactive"),
      changeType: selectedPlatform === 'overview'
        ? (connectedPlatforms > 0 ? "increase" : "neutral")
        : (metrics.length > 0 ? "increase" : "decrease"),
    },
    {
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          {lastUpdated && (
            <p className="text-xs text-silver">Last updated: {lastUpdated}</p>
          )}
        </div>
        <button
        ...
        >
          <svg
            className={`w-4 h-4 text-electric-purple ${loading ? 'animate-spin' : ''}`}
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
