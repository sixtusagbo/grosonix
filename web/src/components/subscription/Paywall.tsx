...
export function Paywall({
  ...
  const handleSelectPlan = async (tier: string, billing: 'monthly' | 'yearly') => {
    if (tier === 'free') {
      onClose();
      return;
    }
      ...
      <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
        <DialogHeader className="relative">
          ...
          <div className="flex items-center justify-center gap-4 mb-8">
            ...
            />
            <span className={`text-sm ${billingPeriod === 'yearly' ? 'text-text-primary font-medium' : 'text-text-secondary'}`}>
              Yearly
            </span>
