...
export function useSocialCache() {
   ...
   */
  const clearCache = useCallback(async (platforms?: string[]) => {
    setLoading(true);
    setError(null);
    ...
    try {
      ...
      const data = await response.json();
      toast.success(`Cache cleared for: ${data.cleared_platforms.join(', ')}`);
   ...
   */
  const warmCache = useCallback(async (platforms?: string[]) => {
    setLoading(true);
    setError(null);
   ...
   */
  const isRateLimited = useCallback((platform: string, endpoint?: string): boolean => {
    ...
    return Object.values(platformLimits).some((limit: any) => limit.limit_reached);
  }, [cacheData]);

  /**
   * Get cache expiry time for a platform
   */
  const getCacheExpiry = useCallback((platform: string): Date | null => {
   ...
   */
  const isCacheExpired = useCallback((platform: string): boolean => {
    const cacheStatus = cacheData?.cache_status[platform];
   ...
   */
  const getTimeUntilReset = useCallback((platform: string, endpoint: string): number => {
    const rateLimitInfo = cacheData?.rate_limits[platform]?.[endpoint];
   ...
   */
  const formatTimeUntilReset = useCallback((platform: string, endpoint: string): string => {
  ...
  const getCacheStats = useCallback(() => {
    ...
    const cached = platforms.filter(p => cacheData.cache_status[p].has_cache).length;
    const expired = platforms.filter(p => cacheData.cache_status[p].is_expired).length;
