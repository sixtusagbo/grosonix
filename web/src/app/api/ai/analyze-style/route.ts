...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!canAnalyze) {
      return Response.json(
        { 
          ...
          message: 'Style analysis limit reached. Upgrade to Pro for unlimited access.' 
        },
        { status: 429 }
      );
    }
      ...
      if (existingProfile) {
        ...
        if (daysSinceAnalysis < 7) {
          return Response.json({
            ...
            analysis_summary: styleAnalyzer.generateStyleSummary(existingProfile),
            from_cache: true,
          });
        }
      }
    ...
    if (!socialAccounts || socialAccounts.length === 0) {
      return Response.json(
        { 
          ...
          message: 'Please connect at least one social media account to analyze your style.' 
        },
        { status: 400 }
      );
    }
    ...
    if (posts.length === 0) {
      return Response.json(
        { 
          ...
          message: 'No posts found to analyze. Please make sure you have recent posts on your connected social accounts.' 
        },
        { status: 400 }
      );
    }
    ...
    return Response.json(
      { error: 'Internal server error', message: 'Failed to analyze writing style' },
      { status: 500 }
    );
  }
}
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return Response.json(
      { error: 'Internal server error', message: 'Failed to retrieve style profile' },
      { status: 500 }
    );
  }
}
